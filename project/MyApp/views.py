...
# Attention:
# 本系统使用python3.6版本，MySQL5.7版本django2.2版本，pycharm3.5专业版,默认使用MySQL数据库，要使用django自带后台管理系统的在settings.py文件里进行修改
...
def doctor_regist(request):
    if request.method == "GET":
        ...
        # print("user_result_u")
        # print(type(user_result_u))
        user_result = Doctor.objects.filter(doctor_name=user_name).first()
        doctor_i = user_result.doctor_id  #doctor表里该医生的编号
...
def add_patient(request):
    if request.method == "GET":
        ...
        Patient.objects.create(
            ...
            patient_telep=patient_telep,
            patient_dept=d)  # 插入注册的用户信息
        patient_result = Patient.objects.filter(
            patient_idcard=patient_idcard).first()
...
def search_p(request):
    ...
    return render(
        request,
        'Patient/search_patient_choose.html',
        context=context)
...
def search_by_patient_name(request):
    ...
    return render(
        request,
        'Patient/search_by_patient_name.html',
        context=context)
...
def search_by_patient_dept(request):
    if request.method == "GET":
        ...
        patient_dept = request.POST.get("patient_dept")
        patient_infos = Patient.objects.filter(
            patient_dept=int(patient_dept)).all()
        # print(patient_infos)
        context = {
    ...
    return render(
        request,
        'Patient/search_by_patient_dept.html',
        context=context)
...
def search_by_doctor_name(request):
    ...
    return render(
        request,
        'Doctor/search_by_doctor_name.html',
        context=context)
...
def search_by_doctor_dept(request):
    ...
    return render(
        request,
        'Doctor/search_by_doctor_dept.html',
        context=context)
...
def alter(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            'Doctor/alter_doctor_inform.html',
            context=context)
...
def alter_work_time(request):
    ...
    Work.objects.filter(
        work_doctor_name=id4,
        work_time=alter_value).update(
        work_time=alter_value_to)
    work_time_results = Work.objects.filter(work_doctor_name=id4)
    context = {
        "work_time_results": work_time_results,
    }
...
def add_medicine(request):
    ...
    if request.method == "GET":
        ...
        Medicine.objects.filter(
            medicine_name=medicine1).update(
            medicine_num=num_result)
        context = {
            "num": num,
            "medicine_": medicine1
        }
...
def display(request):  # 分区显示
    global id1, info
    patient_id = request.POST.get("patient_id")
    # print(44)
    # print(patient_id)
    # print(type(patient_id))  # <class 'App.models.Patient'>
    id1 = patient_id
    patient_info = Patient.objects.filter(id=int(patient_id)).first()
    # print("patient_info")
    # print(patient_info)  # Patient object
    # print(type(patient_info))  # <class 'App.models.Patient'>
...
def insert_deal_method(request):
    ...
    # <class 'App.models.Patient'>没first()时是：<class 'django.db.models.query.QuerySet'>
    # print(type(d))
    # Patient object  ,没first()时是：<QuerySet [<Patient: Patient object>]>
    # print(d)
    ...
    Deal_method.objects.create(
        ...
        doctor_suggestions=doctor_suggestions)  # 插入注册的用户信息
    p = Patient.objects.filter(id=int(patient_id)).first()
    Room.objects.filter(
        room_patient_dept=p.patient_dept,
        room_id=room_id).update(
        room_patient_id=patient_id,
        room_patient_name=d.patient_name)  # 更新数据
    # Room.objects.filter(room_id=room_id).update(room_patient_name=d.patient_name)
...
def out_hospital(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            'Doctor/out_hospital_regist.html',
            context=context)
    else:
        depts = Dept.objects.all()
        ...
        # print(int(dept1))
        Room.objects.filter(
            room_patient_dept=int(dept1),
            room_id=out_id).update(
            room_patient_name="空",
            room_patient_id="空")  # 更新数据
        dept_room_infos = Room.objects.filter(
            room_patient_dept=int(patient_dept1))
        context = {
            "depts": depts,
            "status": 1,
...
def search_room(request):
    ...
    dept1 = patient_dept1
    # print(patient_dept1)   #1
    # print(type(patient_dept1)) #str
...
def work_time_search(request):
    ...
    if request.method == "GET":
        ...
        work_time_results = Work.objects.filter(
            work_doctor_dept=int(work_dept))
        context = {
            "work_time_results": work_time_results,
...
def distribute_room(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            'Doctor/search_empty_room.html',
            context=context)
    else:
        patient_dept = request.POST.get("patient_dept")
        # print(patient_dept)
        # print(type(patient_dept))  # <class 'str'>
        dept_room_infos = Room.objects.filter(
            room_patient_dept=int(patient_dept))
        # <QuerySet [<Room: Room object>, <Room: Room object>, <Room: Room object>, <Room: Room object>, <Room: Room object>]>
        # print(dept_room_infos)
        # <class 'django.db.models.query.QuerySet'>
        # print(type(dept_room_infos))
        d = {
            "1": "骨科",
            "2": "烧伤科",
            "3": "皮肤科",
